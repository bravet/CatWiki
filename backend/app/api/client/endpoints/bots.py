...
from app.core.integration.robot.wecom_robot.crypt import WXBizJsonMsgCrypt
from app.core.integration.robot.wecom_robot.service import WeComRobotService
...
async def get_wecom_kefu_context(site_id: int = Query(...), db: AsyncSession = Depends(get_db)):
    ...
    config = site.bot_config.get("wecomKefu", {})
    if not config or not config.get("enabled"):
